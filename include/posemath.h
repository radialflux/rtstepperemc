...
struct PM_CARTESIAN
{
 ...
 PM_CARTESIAN(PM_CCONST PM_CARTESIAN & cart):x(cart.x), y(cart.y), z(cart.z)
   {
   };   /* added  7-May-1997 by WPS */
#endif
...
#ifdef INCLUDE_POSEMATH_COPY_CONSTRUCTORS
 PM_QUATERNION(PM_CCONST PM_QUATERNION & quat):s(quat.s), x(quat.x), y(quat.y), z(quat.z)
   {
   };   /* added  7-May-1997 by WPS -- inlined June 26,2006 by WPS */
#endif
   ...
   /* operators */
   double &operator [] (int n); /* this[n] */
   PM_QUATERNION & operator =(PM_CONST PM_QUATERNION & q)
   {
      s = q.s;
      x = q.x, y = q.y;
      z = q.z;
      return (*this);
   };   /* this = q -- inlined June 26,2006 by WPS  */
...
struct PM_RPY
{
   ...
   PM_RPY & operator =(PM_CONST PM_RPY & rpy)
   {
      r = rpy.r;
      p = rpy.p;
      y = rpy.y;
...
struct PM_POSE
{
   ...
   PM_POSE(double x, double y, double z, double s, double sx, double sy, double sz);
...
struct PM_CIRCLE
{
   ...
   /* functions */
   int init(PM_POSE start, PM_POSE end, PM_CARTESIAN center, PM_CARTESIAN normal, int turn);
...
/* circle functions */
   ...
   extern int pmCircleInit(PmCircle * circle, PmPose start, PmPose end, PmCartesian center, PmCartesian normal, int turn);
