       ...
       (scanned == 8 || scanned == 9)) {
        if(!random_toolchanger) {
            ...
            if(fakepocket >= CANON_POCKETS_MAX) {
                BUG("too many tools. skipping tool %d\n", toolno);
                return true;
            }
            if(fms)
        ...
        if (pocket < 0 || pocket >= CANON_POCKETS_MAX) {
            BUG("max pocket number is %d. skipping tool %d\n", CANON_POCKETS_MAX-1, toolno);
            return true;
        } else {
            /* lathe tool */
                         ...
                         int random_toolchanger)
{
    ...
    if (NULL == (fp = fopen(filename, "r"))) {
        // can't open file
        BUG("warning unable to open tool table %s\n", filename);
//        emcOperatorError(0, EMC_I18N("warning unable to open tool table %s"), filename);
        return -1;
    }
    ...
    */
    ...
    while (!feof(fp)) {
        ...
        if(scan_old_style(buffer, toolTable, fms, ttcomments, random_toolchanger, fakepocket))
            continue;
        ...
        if (valid) {
            ...
            BUG("Unrecognized line skipped: %s", orig_line);
        }
