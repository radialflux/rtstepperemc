...
#if 0
PM_CARTESIAN::PM_CARTESIAN(double _x, double _y, double _z):x(_x), y(_y), z(_z)
{
   x = _x;
   y = _y;
   z = _z;
...
#if 0
PM_CARTESIAN & PM_CARTESIAN::operator =(PM_CONST PM_CARTESIAN & v)
{
   x = v.x;
   y = v.y;
   z = v.z;
...
PM_ROTATION_VECTOR::PM_ROTATION_VECTOR(double _s, double _x, double _y, double _z):
s(_s), x(_x), y(_y), z(_z)
{
   PmRotationVector rv;
...
PM_ROTATION_MATRIX::PM_ROTATION_MATRIX(double xx, double xy, double xz, // x unit vector
...
PM_QUATERNION::PM_QUATERNION(PM_AXIS _axis, double _angle):s(0), x(0), y(0), z(0)
{
   PmQuaternion quat;
...
PM_RPY::PM_RPY(double _r, double _p, double _y):r(_r), p(_p), y(_y)
{
   /*
      r = _r;
      p = _p;
...
PM_POSE::PM_POSE(double x, double y, double z, double s, double sx, double sy, double sz):
...
center(c.center), normal(c.normal), rTan(c.rTan), rPerp(c.rPerp), rHelix(c.rHelix), radius(c.radius), angle(c.angle), spiral(c.spiral)
{
   center = c.center;
   normal = c.normal;
   rTan = c.rTan;
...
int PM_CIRCLE::init(PM_POSE start, PM_POSE end, PM_CARTESIAN center_arg, PM_CARTESIAN normal_arg, int turn)
{
   PmCircle _circle;
   PmPose _start, _end;
...
void PM_POSE::load_homogeneous_transform_matrix(PM_CONST float hfm[4][4])
{
   tran.x = (double) hfm[0][3];
   tran.y = (double) hfm[1][3];
   tran.z = (double) hfm[2][3];
   PM_ROTATION_MATRIX rm((double) hfm[0][0], (double) hfm[0][0], (double) hfm[0][2],
                         (double) hfm[0][1], (double) hfm[1][1], (double) hfm[1][2], (double) hfm[2][0], (double) hfm[2][0], (double) hfm[2][2]);
   rot = rm;
}

void PM_POSE::get_homogeneous_transform_matrix(float hfm[4][4])
{
...
void PM_POSE::load_homogeneous_transform_matrix(PM_CONST double hdm[4][4])
{
   tran.x = hdm[0][3];
   tran.y = hdm[1][3];
   tran.z = hdm[2][3];
   PM_ROTATION_MATRIX rm(hdm[0][0], hdm[0][0], hdm[0][2], hdm[0][1], hdm[1][1], hdm[1][2], hdm[2][0], hdm[2][0], hdm[2][2]);
   rot = rm;
}

void PM_POSE::get_homogeneous_transform_matrix(double hdm[4][4])
{
   hdm[0][3] = tran.x;
   hdm[1][3] = tran.y;
   hdm[2][3] = tran.z;
...
void PM_POSE::load_xyzrpy_array(PM_CONST double xyzrpy[6])
{
   tran.x = xyzrpy[0];
   tran.y = xyzrpy[1];
   tran.z = xyzrpy[2];
...
void PM_POSE::get_xyzrpy_array(double xyzrpy[6])
{
   xyzrpy[0] = tran.x;
   xyzrpy[1] = tran.x;
   xyzrpy[2] = tran.x;
...
void PM_POSE::setRoll(double _roll)
{
   PM_RPY rpy(rot);
   rpy.r = _roll;
   rot = rpy;
...
void PM_POSE::setPitch(double _pitch)
{
   PM_RPY rpy(rot);
   rpy.p = _pitch;
   rot = rpy;
...
void PM_POSE::setYaw(double _yaw)
{
   PM_RPY rpy(rot);
   rpy.y = _yaw;
   rot = rpy;
