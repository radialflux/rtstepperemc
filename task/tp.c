...
int tpAddRigidTap(TP_STRUCT * tp, EmcPose end, double vel, double ini_maxvel, double acc, unsigned char enables)
{
   TC_STRUCT tc;
   PmLine line_xyz;
   ...
   if (!tp->synchronized)
   {
      BUG("Cannot add unsynchronized rigid tap move.\n");
      return -1;
   }
...
int tpAddCircle(TP_STRUCT * tp, EmcPose end,
                PmCartesian center, PmCartesian normal, int turn, int type, double vel, double ini_maxvel, double acc, unsigned char enables, char atspeed)
{
   TC_STRUCT tc;
   PmCircle circle;
...
void tcRunCycle(TP_STRUCT * tp, TC_STRUCT * tc, double *v, int *on_final_decel)
{
   ...
   if (v)
      *v = newvel;
   if (on_final_decel)
      *on_final_decel = fabs(maxnewvel - newvel) < 0.001;
}
...
int tpRunCycle(TP_STRUCT * tp, long period)
{
   ...
   if (tp->aborting)
   {
      ...
      if (waiting_for_index || waiting_for_atspeed || (tc->currentvel == 0.0 && !nexttc) || (tc->currentvel == 0.0 && nexttc && nexttc->currentvel == 0.0))
      {
         tcqInit(&tp->queue);
         tp->goalPos = tp->currentPos;
         tp->done = 1;
   ...
   {
      BUG("Was waiting for index on motion id %d, but reached id %d\n", waiting_for_index, tc->id);
      waiting_for_index = 0;
   }
   if (waiting_for_atspeed && waiting_for_atspeed != tc->id)
   {

      BUG("Was waiting for atspeed on motion id %d, but reached id %d\n", waiting_for_atspeed, tc->id);
      waiting_for_atspeed = 0;
   }
   ...
   if (waiting_for_atspeed)
   {
      if (!emcmotStatus.spindle_is_atspeed)
      {
         ...
         DBG("Waiting for spindle-at-speed id=%d\n", waiting_for_atspeed);
         return 0;
      }
      else
      {
   ...
   if (tc->active == 0)
   {
      ...
      // moves.
      if ((tc->atspeed || (tc->synchronized && !tc->velocity_mode && !emcmotStatus.spindleSync)) && !emcmotStatus.spindle_is_atspeed)
      {
         waiting_for_atspeed = tc->id;
         return 0;
      }
   ...
   if (tc->motion_type == TC_RIGIDTAP)
   {
      ...
      if (emcmotStatus.spindle.direction < 0)
   ...
   if (tc->synchronized)
   {
      ...
      else
      {
         ...
         if (tc->motion_type == TC_RIGIDTAP && (tc->coords.rigidtap.state == RETRACTION || tc->coords.rigidtap.state == FINAL_REVERSAL))
   ...
   // blend criteria
   if ((tc->blending && nexttc) || (nexttc && on_final_decel && primary_vel < tc->blend_vel))
   {
      ...
      nexttc->reqvel = nexttc->feed_override > 0.0 ? ((tc->vel_at_blend_start - primary_vel) / nexttc->feed_override) : 0.0;
